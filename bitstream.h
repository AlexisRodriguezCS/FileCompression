...
class ibitstream : public std::istream {
 public:
   ...
   */
  ibitstream()
      : std::istream(NULL), lastTell(0), curByte(0), pos(NUM_BITS_IN_BYTE) {
    this->fake = false;
  }
  /**
  ...
  int readBit() {
    ...
    if (this->fake) {
      ...
      // if just finished bits from curByte or if data read from stream after
      // last readBit()
   ...
   */
  void setFake(bool fake) { this->fake = fake; }
  ...
  long size() {
    if (!is_open()) {
      // error("ibitstream::size: Cannot get size of stream which is not
      // open.");
    }
...
class obitstream : public std::ostream {
 public:
   ...
   */
  obitstream()
      : std::ostream(NULL), lastTell(0), curByte(0), pos(NUM_BITS_IN_BYTE) {
    this->fake = false;
  }
  /**
   ...
   * ------------------------------------
   * If bits remain to be written in curByte, add bit into byte and increment
   * pos Else if end of curByte (or some other write happened), then start a
   * fresh byte at position 0. We write the byte out for each bit (backing up to
   * overwrite as needed), rather than waiting for 8 bits.     This is because
   * the client might make 3 writeBit calls and then start using << so we can't
   * wait til full-byte boundary to flush any partial-byte bits.
   */
  void writeBit(int bit) {
    if (bit != 0 && bit != 1) {
      // error(std::string("obitstream::writeBit: must pass an integer argument
      // of 0 or 1. You passed the integer ")
    ...
    if (this->fake) {
      ...
      // if just filled curByte or if data written to stream after last
      // writeBit()
      ...
      if (pos == 0 ||
          bit) {   // only write if first bit in byte or changing 0 to 1
        if (pos != 0) {
...
class ifbitstream : public ibitstream {
 public:
   ...
   * reading.
   */
  bool is_open() { return fb.is_open(); }
...
class ofbitstream : public obitstream {
 public:
  ...
  void open(const char* filename) {
    ...
    //            //error(std::string("ofbitstream::open: It is potentially
    //            dangerous to write to file ")
    //                //  + filename + ", because that might be your own source
    //                code.  "
    //                  //+ "We are explicitly disallowing this operation.
    //                  Please choose a "
    //                  //+ "different filename.");
    //            setstate(std::ios::failbit);
    //        }
